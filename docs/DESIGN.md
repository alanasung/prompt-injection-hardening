# Design Document: Role Inference From Style Versus Delimiters

## 1. Purpose

This document is the method contract for **Role Inference From Style Versus Delimiters**. It specifies what is
...
One-liner: Measure whether models trust writing style over structural role tags under injection.

Hypothesis: Measure whether models trust writing style over structural role tags under injection.
...
|---|---|---|
| Config schema | `injrob.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `injrob.configs.loader` | Hydra compose API |
| Device | `injrob.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `injrob.models.registry` | Pinned revisions + hardware fit |
| Hooks | `injrob.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `injrob.models.generation` | Chat template path recording |
| Data | `injrob.data` | Synthetic items, splits, manifests |
| Evaluation | `injrob.evaluation` | Metrics, failure harness, protocols |
| Ablation | `injrob.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `injrob.reporting` | Aggregate → MD/TeX/figures |
| Cache | `injrob.cache` | Versioned artifact store |
...
`src/injrob/injrob/` are preserved across regeneration.
